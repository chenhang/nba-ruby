...
def team_dashboard_api(team_id=0, opp_team_id=0, type=TYPES[:base], per_mode=PER_MODE[:per_game])
...
File.open("data/teams_stats.json", "w") do |f|
  f.write(parse(0, 0).to_json)
end

File.open("/root/hu_new_leave.json", "w") do |f|
  f.write(ff.to_json)
end
